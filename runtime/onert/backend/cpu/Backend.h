...
public:
  ...
  std::unique_ptr<onert::backend::BackendContext> newContext(ContextData &&data) const override
  {
    ...
    const auto &operations = data.graph->operations();
    auto context = std::make_unique<BackendContext>(this, std::move(data.graph));
    ...
    context->kernel_gen = std::make_shared<KernelGenerator>(
      operands, operations, tb, tr, data.custom_kernel_builder, context->external_context());
    return context;
  }
